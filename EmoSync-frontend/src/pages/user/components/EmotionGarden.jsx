...
const EmotionGarden = ({
  ...
  const emptyEmotionData = {
    ...
    suggestion:
      "No emotion analysis available yet. Start chatting to see insights.",
    icon: "😐",
    label: "Neutral",
    riskDescription: "",
  ...
  const getFlowerCenterClass = (isNegative) => {
    const baseClass =
      "absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[50px] h-[50px] rounded-full flex flex-col items-center justify-center z-10 shadow-[0_4px_16px_rgba(0,0,0,0.1)] border-2 border-white/80";
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl p-4 mb-5 min-h-[320px]
    ...
    >
      ...
      <div className="flex flex-col  mb-5 relative z-10">
        <div className="flex justify-between gap-2">
          <div className="flex gap-2 text-[#8b4513] font-semibold text-base">
            <i className="fas fa-seedling text-[#90ee90] text-lg animate-sway"></i>
            <FontAwesomeIcon
              icon={faSeedling}
              className="text-[#90ee90] text-lg animate-sway"
            />
        ...
        <div className="flex justify-between gap-2 px-2 text-gray-600 text-xs">
          <span className="flex items-center gap-1 ">
            Updated:
            {lastUpdated && <div>{formatTime(lastUpdated)}</div>}
          </span>
          ...
          >
            <FontAwesomeIcon className="text-md" icon={faSync} />
          </button>
        </div>
      </div>
      ...
      {isRefreshing && (
        <div className="absolute inset-0 bg-white/70 flex items-center justify-center z-20 rounded-2xl">
          <div className="text-center">
            <div
              className="w-8 h-8 border-4 border-t-4 border-gray-200 border-t-blue-500 
      ...
      {emotionData.riskLevel > 0 && (
        <div
          className={`mb-4 p-3 rounded-lg relative z-10 
                        ${
                          emotionData.riskLevel >= 3
                            ? "bg-red-50 border border-red-200"
                            : emotionData.riskLevel >= 2
            ...
            <div>
              ...
              <div className="text-xs opacity-75">
                {emotionData.riskDescription ||
                  getRiskDescription(emotionData.riskLevel)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-6 relative z-10">
        ...
        <div className="flex-1">
          <div className="relative w-[150px] h-[150px] mx-auto mb-4">
            ...
            <div className="relative w-full h-full">
              {[1, 2, 3, 4, 5, 6].map((index) => {
                ...
                switch (index) {
                  case 1:
                    positionClass = "top-[15px] left-[55px]";
                    break;
                  case 2:
                    positionClass = "top-[30px] right-[15px]";
                    break;
                  case 3:
                    positionClass = "bottom-[30px] right-[25px]";
                    break;
                  case 4:
                    positionClass = "bottom-[15px] left-[55px]";
                    break;
                  case 5:
                    positionClass = "bottom-[30px] left-[25px]";
                    break;
                  case 6:
                    positionClass = "top-[30px] left-[15px]";
                    break;
                }

                const animationType = getPetalAnimationClass(
                  emotionData.emotionScore,
                  emotionData.riskLevel
                );
                return (
                  <div
                    key={index}
                    className={`absolute w-[35px] h-[24px] rounded-[50%_50%_50%_0]
                              ${positionClass} ${getPetalClass(
                      emotionData.emotionScore,
                      emotionData.riskLevel
                    )}
          ...
          <div className="text-center">
            <div
              className={`inline-block px-4 py-2 rounded-2xl text-sm font-semibold
                            ${getEmotionStatusClass(
                              emotionData.isNegative,
                              emotionData.riskLevel
                            )}`}
            >
              Current feeling:{" "}
      ...
      {emotionData.improvementSuggestions &&
        emotionData.improvementSuggestions.length > 0 && (
          <div className="relative z-10">
            ...
            <div className="space-y-2">
              {emotionData.improvementSuggestions.map((action, index) => (
                ...
                >
                  <div className="text-sm text-blue-500 flex-shrink-0 mt-0.5">
                    ✓
                  </div>
